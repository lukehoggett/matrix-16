...
#define open_line0                                                             \
  { *portOutputRegister(digitalPinToPort(8)) = digitalPinToBitMask(8); }
#define open_line1                                                             \
  { *portOutputRegister(digitalPinToPort(9)) = digitalPinToBitMask(9); }
#define open_line2                                                             \
  { *portOutputRegister(digitalPinToPort(10)) = digitalPinToBitMask(10); }
#define open_line3                                                             \
  { *portOutputRegister(digitalPinToPort(11)) = digitalPinToBitMask(11); }
#define open_line4                                                             \
  { *portOutputRegister(digitalPinToPort(12)) = digitalPinToBitMask(12); }
#define open_line5                                                             \
  { *portOutputRegister(digitalPinToPort(13)) = digitalPinToBitMask(13); }
#define open_line6                                                             \
  { *portOutputRegister(digitalPinToPort(3)) = digitalPinToBitMask(3); }
#define open_line7                                                             \
  { *portOutputRegister(digitalPinToPort(4)) = digitalPinToBitMask(4); }

#if defined(__AVR_ATmega1280__) || defined(__AVR_ATmega2560__)
#define close_all_lines                                                        \
  {                                                                            \
    PORTB &= 0b00001111;                                                       \
    PORTG &= 0b11011111;                                                       \
    PORTE &= 0b11011111;                                                       \
...
// TCNT2 increments every  = 16MHz / prescaler
    ...
    ASSR |=
        (1 << AS2); // Use internal clock - external clock not used in Arduino
    TIMSK2 |= ((1 << TOIE2) | (0 << OCIE2B)); // Timer2 Overflow Interrupt
                                              // Enable
    TCNT2 = 0xff;
#endif
    sei();
  ...
  // get a pixel for writing in the offscreen framebuffer, return null if out of
  // screen
  PixelRGB *GetPixel(char x, char y) {
    if (x < 0 || x >= ColorduinoScreenWidth || y < 0 ||
        y >= ColorduinoScreenHeight) {
      return NULL;
    }
  ...
  void SetPixel(char x, char y, unsigned char r, unsigned char g,
                unsigned char b) {
